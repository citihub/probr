...
func (s *scenarioState) thePodIsDeployedSuccessfully() error {
	...
	description := fmt.Sprintf("The Pod name deploying is %s ", s.podState.PodName)
	payload := struct {
		PodState kubernetes.PodState
		PodName  string
	}{s.podState, s.podState.PodName}
...
func (s *scenarioState) anAttemptToObtainAnAccessTokenFromThatPodShouldFail() error {
	...
	description := fmt.Sprintf("Attempting to get access from Pod is %s ", s.podState.PodName)
	payload := struct {
		PodState kubernetes.PodState
		PodName  string
	}{s.podState, s.podState.PodName}
...
func (s *scenarioState) iExecuteTheCommandAgainstTheMICPod(arg1 string) error {
	...
	description := fmt.Sprintf("Executing command is %s,", arg1)
	payload := struct {
		PodState kubernetes.PodState
		PodName  string
	}{s.podState, s.podState.PodName}
