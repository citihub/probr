...
func (ctx *CVars) SetTags(tags map[string][]string) {
...
func (ctx *CVars) handleTagExclusions() {
...
func NewConfig(c string) (CVars, error) {
...
func (ctx *CVars) AuditDir() string {
...
func (ctx *CVars) CucumberDir() string {
...
func (ctx *CVars) handleProbeExclusions(packName string, probes []Probe) {
...
func (ctx *CVars) addExclusion(tag string) {
	if len(ctx.Tags) > 0 {
		ctx.Tags = ctx.Tags + " && "
	}
	ctx.Tags = fmt.Sprintf("%s~@%s", ctx.Tags, tag)
