...
func newConfigWithScenarioExclusionAndInclusion() (config CVars, excludedTag string) {
	config, _ = NewConfig("")
...
func checkPreformattedScenarioExclusions(config CVars, t *testing.T) {
...
func checkTagsContainExclusion(config CVars, tag string, t *testing.T) {
...
func TestNewConfig(t *testing.T) {
	...
	case CVars:
	default:
		t.Log("NewConfig did not create a CVars object")
		t.Fail()
	}
}
...
func TestSetTags(t *testing.T) {
	...
	if expected != 2 {
		t.Errorf("Tag name and tag ID were not found in CVars.Tags")
	}
}
